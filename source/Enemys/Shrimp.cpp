...
void Shrimp::update(float dt) {
    ...
    if (_killMeCountdown != 0.0f) {
        if (getRequestedAction().find("death") != std::string::npos) {
            return;
		}
        if (_state == "standing" || _state == "standingRunning") {
            CULog("STANDING DEATH");
            setRequestedActionAndPrio("shrimpStandDeath", 1000);
        }
        else if (_state == "curling" || _state == "rolling" || _state == "uncurling" || _state == "stunned") {
            CULog("ROLLING DEATH");
            setRequestedActionAndPrio("shrimpRollDeath", 1000);
        }
        else {
            CULog("IDLE DEATH");
            setRequestedActionAndPrio("shrimpIdleDeath", 1000);
        }
    }
        ...
        setRequestedActionAndPrio("shrimpAlarm", 40);
    }
    else if (_state == "standing") {
        setRequestedActionAndPrio("shrimpStand", 41);
    }
    else if (_state == "standingRunning") {
        setRequestedActionAndPrio("shrimpRun", 42);
    }
        ...
        if (getActiveAction() != "shrimpIdle" && getActiveAction() != "shrimpWalk" && getActiveAction() != "shrimpWalkStart" && getActiveAction() != "shrimpUncurl") {
            setRequestedActionAndPrio("shrimpUncurl", 30);
        }
        else {
            if (velocity.x != 0) {
        ...
        setRequestedActionAndPrio("shrimpRespawn", 1000);
    }
    else {
        CULog("error: shrimp");
        CULog(_state.c_str());
...
void Shrimp::fixedUpdate(float step) {
    ...
    b2Vec2 velocity = _body->GetLinearVelocity();
    if (_state != "rolling" && _state != "stunned" || (_killMeCountdown != 0.0f)) {
        setAngle(0.0f);
        setFixedRotation(true);
    }
    if (_killMeCountdown != 0.0f) {
    ...
    if (_state == "curling" && getSpriteNode()->getFrame() == 5) _body->ApplyLinearImpulse(b2Vec2(0, 10.0f), _body->GetWorldCenter(), true);
}

b2Vec2 Shrimp::handleMovement(b2Vec2 velocity) {
...
void Shrimp::setState(std::string state) {
    ...
    if (_state == "stunned" && state == "rolling") _body->SetAngularVelocity(0.0f);
    if (state == "curling") {
        _behaviorCounter = getActionDuration("shrimpCurl");
    }
    else if (state == "alarm") {
        _behaviorCounter = getActionDuration("shrimpAlarm");
    }
    else if (state == "standing") {
        _behaviorCounter = getActionDuration("shrimpStand");
    }
    else if (state == "standingRunning") {
        _behaviorCounter = -1;
    }
    else if (state == "uncurling") {
        _behaviorCounter = getActionDuration("shrimpUncurl");
    }
    else if (state == "rolling") {
        _behaviorCounter = -1;
    }
        ...
        _behaviorCounter = getActionDuration("shrimpRespawn");
    }
}